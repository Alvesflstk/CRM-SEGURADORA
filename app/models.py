...
class table_investimentos(db.Model):
...
def calcular_ivestimentos():
    soma = db.session.query(db.func.sum(table_investimentos.valor)).scalar()
    return soma

def calcular_comicao():
    finish_dado = db.session.query(db.func.sum(table_vendas.comissao)).scalar()
    return finish_dado
